...
/*
	 ...
	 func InitMyParamTable() {
		...
		MyParamTable.RegisterCalc(_FIRST_CALC, func(t *CalcInterface) {
			vala := t.GetInput_U64(_INPUT_FIRST_CALC_A) // first input
			valb := t.GetInput_I64(_INPUT_FIRST_CALC_B) // second input
			valout := int32(vala) + int32(valb)
			t.SetOutput_I32(_OUT_FIRST_CALC, valout)
		})
		// Init root values
		MyParamTable.InitRoot_U64(FIRST_U64_PARAM, 1, false)
		MyParamTable.InitRoot_I64(FIRST_I64_PARAM, -2, true)
		// Init derived values
		initFirstCalcDerived(&MyParamTable, false, FIRST_U64_PARAM, FIRST_I64_PARAM, FIRST_I32_PARAM)
	 }
*/
